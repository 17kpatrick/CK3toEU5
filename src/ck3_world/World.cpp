...
// CK3 1.19 sometimes writes character=4294967295 (uint32 max) when the live ID isn't available.
constexpr long long invalidPlayedCharacterSentinel = 4294967295LL;

[[nodiscard]] bool isUsablePlayedCharacterID(long long characterID)
{
	return characterID > 0 && characterID != invalidPlayedCharacterSentinel;
}
} // namespace
...
CK3::World::World(const configuration::Configuration& theConfiguration, const commonItems::ConverterVersion& converterVersion)
{
	...
	// Named colors must be primed before any coat of arms is parsed. The melted meta can carry
	// the player's CoA (and on some saves, rakaly's meltMeta returns enough of the document that
	// the full coat_of_arms table is walked here) - without the dictionary every color1/2/3 name
	// falls back to black and floods the log.
...
void CK3::World::registerKeys(const configuration::Configuration& theConfiguration, const commonItems::ConverterVersion& converterVersion)
{
	...
	registerKeyword("currently_played_characters", [this](std::istream& theStream) {
		...
		if (!playedCharacters.empty() && isUsablePlayedCharacterID(playedCharacters[0]))
			playerID = playedCharacters[0];
	});
	registerKeyword("played_character", [this](std::istream& theStream) {
		// CK3 1.19 often drops currently_played_characters from the melt. played_character still
		// carries the live ID when valid, otherwise the latest legacy.character entry (the ruler
		// the player was controlling when they died / when the ID went missing).
		if (playerID)
		{
			commonItems::ignoreItem("played_character", theStream);
			return;
		}

		long long directID = 0;
		long long legacyID = 0;
		commonItems::parser playedParser;
		playedParser.registerKeyword("character", [&directID](std::istream& stream) {
			directID = commonItems::singleLlong(stream).getLlong();
		});
		playedParser.registerKeyword("legacy", [&legacyID](std::istream& stream) {
			for (const auto& blob: commonItems::blobList(stream).getBlobs())
			{
				std::stringstream legacyStream(blob);
				commonItems::parser legacyParser;
				legacyParser.registerKeyword("character", [&legacyID](std::istream& characterStream) {
					const auto candidate = commonItems::singleLlong(characterStream).getLlong();
					if (isUsablePlayedCharacterID(candidate))
						legacyID = candidate; // last usable entry wins - the most recent player character
				});
				legacyParser.registerRegex(commonItems::catchallRegex, commonItems::ignoreItem);
				legacyParser.parseStream(legacyStream);
			}
		});
		playedParser.registerRegex(commonItems::catchallRegex, commonItems::ignoreItem);
		playedParser.parseStream(theStream);

		if (isUsablePlayedCharacterID(directID))
			playerID = directID;
		else if (isUsablePlayedCharacterID(legacyID))
			playerID = legacyID;
	});
	...
	registerKeyword("traits_lookup", [this](const std::string&, std::istream& theStream) {
		// The save's own trait table: a character's trait numbers are 0-based positions in this
		// list. Authoritative over anything scraped from game files - it reflects the exact game
		// and mods the save was played with.
		traitsLookup = commonItems::stringList(theStream).getStrings();
		Log(LogLevel::Info) << "<> Loaded " << traitsLookup.size() << " traits from the save's lookup table.";
	});
...
void CK3::World::processSave(const std::filesystem::path& saveGamePath, bool debug)
{
	...
	// Exit autosaves come in a compressed layout the melter rejects; repacked, they read like any
	// other binary save. Everything else passes through untouched.
	saveGame.gamestate = repackSplitBinarySave(inStream.str());
...
void CK3::World::crosslinkDatabases()
{
	...
	characters.linkTraits(traitsLookup, traitScraper);
...
{
	// Locates every independent ruler's councilors and knights - the people who become EU5 cabinet
	// members and generals. It's anything but trivial, as being employed doesn't equate with being
	// a councilor, nor do landed councilors have employers if they work for their liege.
	...
	for (const auto& character: characters.getCharacters())
	{
		...
		{
			// Being employed but without a council task means a knight or physician or similar.
	...
	Log(LogLevel::Info) << "<> " << counterAdvisors << " councilors detained, " << counterKnights << " knights armed.";
}
