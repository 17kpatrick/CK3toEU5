...
void CK3::Title::registerKeys()
{
	...
	nameParser.registerKeyword("article", [this](const std::string&, std::istream& theStream) {
		article = commonItems::singleString(theStream).getString();
	});
	...
	const auto parseTitleNameData = [this](std::istream& theStream) {
		nameParser.parseStream(theStream);
	};
	registerKeyword("title_name_data", parseTitleNameData);
	// CK3 1.19+: rakaly doesn't know the title_name_data token, so melted saves write jens_153 instead.
	// Without this alias every dynamic title falls back to its internal key (x_script_8789 Empire).
	registerKeyword("jens_153", parseTitleNameData);
	...
	/*
	 // Disabled: CK3 started writing full utf-8 map names that break when non-ascii.
	 // Retaining the parse path in case a safe encoding becomes available later.
...
void CK3::Title::pickDisplayName(const std::map<std::string, std::shared_ptr<Title>>& possibleTitles)
{
	...
	// If the title's name is transferring, make sure it makes sense. Thrace should use Constantinople's name...
