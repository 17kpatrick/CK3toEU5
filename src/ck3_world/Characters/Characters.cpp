...
void CK3::Characters::linkTraits(const std::vector<std::string>& traitsLookup, const mappers::TraitScraper& traitScraper)
{
	// A character's trait numbers are 0-based positions in the save's own traits_lookup table -
	// the authoritative source, since it reflects the exact game version and mods the save was
	// played with. Scraped game files only stand in for saves too old to carry the table.
	if (traitsLookup.empty())
		Log(LogLevel::Warning) << "Save carries no traits_lookup table; falling back to traits scraped from game files.";
	auto counter = 0;
	for (const auto& character: characters)
	{
		...
		{
			if (trait.first >= 0 && static_cast<size_t>(trait.first) < traitsLookup.size())
				translatedTraits.insert(std::pair(trait.first, traitsLookup[trait.first]));
			else if (const auto& traitMatch = traitScraper.getTraitForID(trait.first); traitMatch)
