...
void EU5::World::setupGovernment(const std::shared_ptr<CK3::Character>& holder, Country& country) const
{
	...
	// Vanilla ships landlocked / minority-gated twins of most templates. Coastal ones hand out
	// maritime privileges an inland realm can never satisfy; muslim_monarchy grants Abrahamic
	// dhimmi rights that need Christian/Jewish majority locations. Celtic tribes are the one
	// case that legitimately wants gaelic_tribe (Hibernian privileges + celtic_traditions_reform).
	if (!country.templateInclude.empty())
	{
		...
		if (category == "tribe" && (gameDatabase.isCultureInGroup(country.culture, "celtic_group") ||
												 gameDatabase.isCultureInGroup(country.culture, "hibernian_group")) &&
			 gameDatabase.hasTemplate("gaelic_tribe"))
		{
			country.templateInclude = "gaelic_tribe";
			country.parliamentType = "assembly";
			country.heirSelection = "tanistry_elective";
		}

		if (!coastal && gameDatabase.hasTemplate(country.templateInclude + "_no_coast"))
			country.templateInclude += "_no_coast";

		// muslim_monarchy(_no_coast) -> muslim_monarchy_no_abrahamic_dhimmi(_no_coast) when the
		// realm has no Christian/Jewish-majority locations for dhimmi_abrahamic_communities.
		if (!hasAbrahamicMinorityLocations(country))
		{
			std::string abrahamicTwin = country.templateInclude;
			constexpr std::string_view noCoast = "_no_coast";
			if (abrahamicTwin.ends_with(noCoast))
				abrahamicTwin.insert(abrahamicTwin.size() - noCoast.size(), "_no_abrahamic_dhimmi");
			else
				abrahamicTwin += "_no_abrahamic_dhimmi";
			if (gameDatabase.hasTemplate(abrahamicTwin))
				country.templateInclude = std::move(abrahamicTwin);
		}
	}
...
bool EU5::World::hasAbrahamicMinorityLocations(const Country& country) const
{
	for (const auto& location: country.locations)
	{
		const auto detail = locationDetails.find(location);
		if (detail == locationDetails.end() || detail->second.religion.empty())
			continue;
		if (detail->second.religion == country.religion)
			continue;
		const auto group = gameDatabase.getReligionGroup(detail->second.religion);
		if (group == "christian" || group == "israelite_group")
			return true;
	}
	return false;
}
...
std::string EU5::World::convertCulture(const std::shared_ptr<CK3::Culture>& culture, const std::string& location)
{
	...
	// No direct match (dynamic/hybrid or renamed culture). Fall back on heritage, then language:
	// if the location's vanilla culture belongs to one of those EU5 groups, local continuity wins.
	auto groups = cultureMapper.getEU5GroupsForHeritage(culture->getHeritage());
	if (groups.empty())
		groups = cultureMapper.getEU5GroupsForLanguage(culture->getLanguage());
...
std::optional<std::string> EU5::World::generateDynamicCulture(const CK3::Culture& culture)
{
	...
	// Language first from the culture's declared language, then from its name lists (hybrids often
	// keep a name list after reforming away from a mapped language string).
	std::optional<std::string> eu5Language = cultureMapper.getEU5LanguageForCK3Language(culture.getLanguage());
	if (!eu5Language)
		for (const auto& nameList: culture.getNameLists())
			if (eu5Language = cultureMapper.getEU5LanguageForNameList(nameList); eu5Language)
				break;
	auto groups = cultureMapper.getEU5GroupsForHeritage(culture.getHeritage());
	if (groups.empty())
		groups = cultureMapper.getEU5GroupsForLanguage(culture.getLanguage());
