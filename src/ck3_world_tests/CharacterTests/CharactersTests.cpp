...
TEST(CK3World_CharactersTests, savesTraitTableTrumpsScrapedFilesAndIsZeroBased)
{
	std::stringstream input;
	input << "1 = {\n";
	input << "\tfirst_name = Alice\n";
	...
	// The scraper disagrees with the save's table; the table must win.
	std::stringstream input2;
	input2 << "wrong0 = {}\n";
	input2 << "wrong1 = {}\n";
	input2 << "wrong2 = {}\n";
	...
	characters.linkTraits({"brave", "craven", "diligent"}, traitScraper);

	const auto& c1 = characters.getCharacters().find(1);
	ASSERT_EQ(2, c1->second->getTraits().size());
	EXPECT_EQ("brave", c1->second->getTraits().find(0)->second);
	EXPECT_EQ("diligent", c1->second->getTraits().find(2)->second);
}
