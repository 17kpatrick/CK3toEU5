...
TEST(CK3World_TitleTests, meltedTitleNameDataAliasCarriesDisplayNameAndAdjective)
{
	// CK3 1.19 melts title_name_data as the unknown token jens_153. Without reading it, dynamic
	// titles ship as x_script_8789 on the EU5 map.
	std::stringstream input;
	input << "key=\"x_script_8789\"\n";
	input << "holder=117679630\n";
	input << "jens_153={\n";
	input << "\tname=\"León\"\n";
	input << "\tadj=\"Leonese\"\n";
	input << "\tarticle=\"the \"\n";
	input << "}\n";
	const CK3::Title theTitle(input, 22439);

	EXPECT_EQ("x_script_8789", theTitle.getName());
	EXPECT_EQ("León", theTitle.getDisplayName());
	EXPECT_EQ("Leonese", theTitle.getAdjective());
}

TEST(CK3World_TitleTests, classicTitleNameDataStillWorks)
{
	std::stringstream input;
	input << "key=\"x_script_8793\"\n";
	input << "holder=67381079\n";
	input << "title_name_data={\n";
	input << "\tname=\"Flail of St. George\"\n";
	input << "}\n";
	const CK3::Title theTitle(input, 22443);

	EXPECT_EQ("Flail of St. George", theTitle.getDisplayName());
}
