...
TEST(Mappers_TraitScraperTests, traitsWithoutIndexAreSelfEnumeratedFromZero)
{
	// Save trait numbers are 0-based positions in declaration order.
	std::stringstream input;
	input << "trait1 = {}\n";
	...
	EXPECT_EQ("trait1", traitScraper.getTraits().find(0)->second);
	EXPECT_EQ("trait2", traitScraper.getTraits().find(4)->second);
	EXPECT_EQ("trait3", traitScraper.getTraits().find(1)->second);
}
...
TEST(Mappers_TraitScraperTests, traitsWithIndexZeroAreSelfEnumerated)
{
	...
	EXPECT_EQ("trait1", traitScraper.getTraits().find(0)->second);
	EXPECT_EQ("trait2", traitScraper.getTraits().find(1)->second);
}
