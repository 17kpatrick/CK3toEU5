...
TEST(Mappers_CultureMapperTests, misspelledLanguageKeywordStillMaps)
{
	std::stringstream input;
	input << "link = { eu5 = netherlandish_group langauge = language_dutch }\n";
	mappers::CultureMapper mapper;
	mapper.loadCultureGroupsMappingRules(input);

	const auto groups = mapper.getEU5GroupsForLanguage("language_dutch");
	ASSERT_EQ(1, groups.size());
	EXPECT_EQ("netherlandish_group", groups[0]);
}
